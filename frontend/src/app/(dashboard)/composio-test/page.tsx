...
export default function ComposioTestPage() {
  ...
  const [testMode, setTestMode] = useState<'auth_config' | 'toolkit'>(
    'toolkit',
  );

  const handleTestAuthentication = async () => {
    if (
      testMode === 'auth_config' &&
      (!authConfigId.trim() || !userId.trim())
    ) {
    ...
    try {
      ...
      if (testMode === 'toolkit') {
        ...
        response = await backendApi.post(
          `/composio/test-toolkit-authentication?${queryParams}`,
        );
      } else {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.test_status === 'success') {
          ...
          toast.error(
            `Test failed: ${response.data.error || response.data.message}`,
          );
        }
      } else {
        const errorMessage =
  ...
  const handleDemoTest = async () => {
    ...
    try {
      ...
      if (testMode === 'toolkit') {
        ...
        response = await backendApi.post(
          `/composio/demo/test-toolkit?${queryParams}`,
        );
      } else {
      ...
      if (response.success && response.data) {
        ...
        if (response.data.test_status === 'success') {
          ...
          toast.error(
            `Demo test failed: ${response.data.error || response.data.message}`,
          );
        }
      } else {
        const errorMessage =
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Test Composio authentication flows - automatically get MCP server URLs
          with successful connections!
        </p>
      </div>
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Choose between auth config ID (manual) or toolkit name
              ...
              <CardContent className="space-y-6">
                <Tabs
                  value={testMode}
                  onValueChange={(value) =>
                    setTestMode(value as 'auth_config' | 'toolkit')
                  }
                >
                  ...
                  <TabsContent value="toolkit" className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        🚀 <strong>Auto-creates auth config!</strong> Just pick
                        any app from 500+ supported toolkits.
                      </p>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="auth_config" className="space-y-4">
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        The pre-created auth configuration ID from your Composio
                        dashboard.
                      </p>
                    </div>
                  </TabsContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm space-y-1">
                  <p>
                    <strong>Toolkit:</strong> github, gmail, slack
                  </p>
                  <p>
                    <strong>User ID:</strong> 0000-1111-2222
                  </p>
                  <p>
                    <strong>Auth Config ID:</strong> ac_dqYN9oElNVlg
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {testResult.auth_config_info.auth_scheme}
                      </Badge>
                    </CardTitle>
                  </CardHeader>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {testResult.toolkit_info.auth_scheme}
                      </Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            copyToClipboard(testResult.toolkit_info!.toolkit)
                          }
                        >
                          <Copy className="h-3 w-3" />
                        </Button>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            copyToClipboard(testResult.toolkit_info!.user_id)
                          }
                        >
                          <Copy className="h-3 w-3" />
                        </Button>
              ...
              {testResult.connection_request && (
                <Card
                  className={
                    testResult.mcp_urls ? 'lg:col-span-2' : 'lg:col-span-2'
                  }
                >
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      Connection Request
                      <Badge variant="outline">
                        {testResult.connection_request.status}
                      </Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
              ...
              {testResult.mcp_urls && (
                <Card className="lg:col-span-2">
                  <CardHeader>
                    ...
                    <CardDescription>
                      These MCP server URLs were automatically generated after
                      your successful connection.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {/* MCP Server Info */}
                    <div className="bg-muted/30 p-3 rounded-lg space-y-3">
                      ...
                      {testResult.mcp_urls.mcp_server_info.allowed_tools
                        .length > 0 && (
                        <div>
                          <Label className="text-xs font-medium text-muted-foreground">
                            Allowed Tools
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {testResult.mcp_urls.mcp_server_info.allowed_tools.map(
                              (tool, index) => (
                                <Badge
                                  key={index}
                                  variant="secondary"
                    ...
                    {testResult.mcp_urls.user_ids_url.length > 0 && (
                      <div>
                        <Label className="text-sm font-medium">
                          🎯 User-Specific URLs (Recommended)
                        </Label>
                        <p className="text-xs text-muted-foreground mb-2">
                          URLs configured for your user ID with the connected
                          account - use these in your MCP client!
                        </p>
                    ...
                    {testResult.mcp_urls.connected_account_urls.length > 0 && (
                      <div>
                        ...
                        {testResult.mcp_urls.connected_account_urls.map(
                          (url, index) => (
                            <div key={index} className="mb-2">
                              <Label className="text-xs font-medium text-muted-foreground">
                                Account URL {index + 1}
            ...
            <Alert
              variant={
                testResult.test_status === 'success' ? 'default' : 'destructive'
              }
            >
              {testResult.test_status === 'success' ? (
              ...
              <AlertDescription>
                <div className="space-y-2">
                  ...
                  {testResult.instructions && (
                    <p>
                      <strong>Next Steps:</strong> {testResult.instructions}
                    </p>
                  )}
                  {testResult.error && (
